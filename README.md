...
设置接收串口4Kbyte缓存数量。
ctrlInfo:setCOM4KbyteNum,5
Win操作系统接收串口数据最大4Kbyte后就会通过网络发送给客户端，
客户端即便上了环形缓冲区，在WIFI网络环境下也会丢数据，网线就不容易丢数据。
故此制作了这个命令用来缓存指定数量个4Kbyte后通过网络发送出去。
不能理解的话，客户端是WIFI无线的情况下可以设置成提供的最大值，客户端是网线的情况下可以设置成0或任意设置。
